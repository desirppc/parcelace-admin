...
interface CourierPartnerSelectionProps {
  ...
  initialShipRates?: ShipRate[]; // Add this prop to accept pre-fetched rates
}
...
const CourierPartnerSelection: React.FC<CourierPartnerSelectionProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (initialShipRates && initialShipRates.length > 0) {
      console.log('Using initialShipRates instead of API call:', initialShipRates);
      setShipRates(initialShipRates);
      return;
    }
    ...
    const fetchRates = async () => {
      ...
      try {
        ...
        console.log('API URL being called:', `${import.meta.env.VITE_API_URL || 'https://app.parcelace.io/'}api/shipments/courier-partner-rates`);
        ...
        try {
          ...
          console.log('API Response shiprates:', data.data?.shiprates);
          console.log('Number of shiprates:', data.data?.shiprates?.length);
          if (data.data?.shiprates) {
            data.data.shiprates.forEach((rate, index) => {
              console.log(`Shiprate ${index}:`, rate);
              console.log(`  - Name: ${rate.name}`);
              console.log(`  - Rates count: ${rate.rate?.length || 0}`);
              if (rate.rate) {
                rate.rate.forEach((r, rIndex) => {
                  console.log(`    Rate ${rIndex}: ${r.name} - ₹${r.totalPayable}`);
                });
              }
            });
          }
