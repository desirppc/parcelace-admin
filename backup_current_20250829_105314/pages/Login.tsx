...
const Login = () => {
  ...
  // Global keyboard event listener for Enter key - only when form is focused
  useEffect(() => {
    const handleGlobalKeyPress = (e: KeyboardEvent) => {
      // Only handle Enter key if we're in a form context and not loading
      if (e.key === 'Enter' && !isLoading) {
        ...
        console.log('🔍 Active element:', activeElement?.tagName, 'Is in form:', isInForm);
        
        if (isInForm) {
          console.log('✅ Form context detected, handling Enter key');
          e.preventDefault();
          e.stopPropagation();
          // Try to submit the form first, fallback to direct call
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      // Try to submit the form first, fallback to direct call
      if (formRef.current && typeof formRef.current.requestSubmit === 'function') {
        console.log('📝 Input: Submitting form via requestSubmit');
        formRef.current.requestSubmit();
      } else {
        console.log('📝 Input: Calling handleLogin directly');
        handleLogin();
      }
    }
  };
