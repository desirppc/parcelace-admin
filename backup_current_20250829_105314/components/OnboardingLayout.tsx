...
const OnboardingLayout = () => {
  ...
  // Auto-close sub-menus when sidebar is closed
  useEffect(() => {
    if (sidebarOpen) {
      hasBeenOpenRef.current = true;
    } else if (hasBeenOpenRef.current) {
      // Only clear expanded menus if sidebar was previously open
      setExpandedMenus([]);
    }
  }, [sidebarOpen]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-blue-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex">
      ...
      >
        ...
        <div className="py-4">
          {menuItems.map((item) => (
            <div key={item.id} data-sidebar="menu-item">
              <button
                onClick={() => {
              ...
              {sidebarOpen && item.subItems && expandedMenus.includes(item.id) && (
                <div className="ml-8 space-y-1 animate-fade-in" data-sidebar="sub-menu">
